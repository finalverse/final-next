...
#include "Tutorial_Hlms02_MyHlmsPbs.h"
...
//-----------------------------------------------------------------------------
void Hlms02CustomizationPerObjGameState::createScene01()
{
    ...
    for( int i = 0; i < 4; ++i )
    {
        for( int j = 0; j < 4; ++j )
        {
            ...
            if( i == 3u && j == 3u )
            {
                item->getSubItem( 0 )->setCustomParameter( Ogre::MyHlmsPbs::kWindId,
                                                           Ogre::Vector4::ZERO );
            }
...
//-----------------------------------------------------------------------------
void Hlms02CustomizationPerObjGameState::update( float timeSinceLast )
{
    OGRE_ASSERT_HIGH( dynamic_cast<Hlms02CustomizationPerObjGraphicsSystem *>( mGraphicsSystem ) );
    Hlms02CustomizationPerObjGraphicsSystem *graphicsSystem =
        static_cast<Hlms02CustomizationPerObjGraphicsSystem *>( mGraphicsSystem );
    ...
    myHlmsListener->mWind.x = ( cosf( mAccumWindTime * 5.0f ) * 0.5f + 0.5f ) * 0.25f;
    mAccumWindTime += timeSinceLast;
    mAccumWindTime = fmodf( mAccumWindTime, Ogre::Math::TWO_PI );

    TutorialGameState::update( timeSinceLast );
}
//-----------------------------------------------------------------------------
void Hlms02CustomizationPerObjGameState::generateDebugText( float timeSinceLast, Ogre::String &outText )
{
...
//-----------------------------------------------------------------------------
void Hlms02CustomizationPerObjGameState::keyReleased( const SDL_KeyboardEvent &arg )
{
