...
namespace Final
{
    ...
    void DistortionGameState::createScene01()
    {
        ...
        */
        ...
        //----------------------------------------------------
        /*
        We will create varous items that are used to distort the scene in postprocessing. You can use
        whatever objects you want to (fe. particle effect billboards) but we are going to create just
        simple spheres.
        */
        {
            // Lets setup a new render queue for distortion pass. Set ID 16 to be our distortion queue
            mGraphicsSystem->getSceneManager()->getRenderQueue()->setRenderQueueMode(
                16, Ogre::RenderQueue::FAST );
            ...
            for( int i = 0; i < 10; ++i )
            {
                ...
                */
                ...
                Ogre::Root *root = mGraphicsSystem->getRoot();
                Ogre::TextureGpuManager *textureMgr = root->getRenderSystem()->getTextureGpuManager();
                // Use non-color data as texture type because distortion is stored as x,y vectors to
                // texture.
                Ogre::TextureGpu *texture = textureMgr->createOrRetrieveTexture(
                    "distort_deriv.png", Ogre::GpuPageOutStrategy::Discard, 0,
                    Ogre::TextureTypes::Type2D,
                    Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME );
                datablock->setTexture( 0, texture );

                // Set material to use vertex colors. Vertex colors are used to control distortion
                // intensity (alpha value)
                datablock->setUseColour( true );
                // Random alpha value for objects. Alpha value is multiplier for distortion strenght
                datablock->setColour(
                    Ogre::ColourValue( 1.0f, 1.0f, 1.0f, Ogre::Math::RangeRandom( 0.25f, 0.5f ) ) );
                ...
                mDistortionSceneNode[i]->setPosition( Ogre::Math::RangeRandom( -3.0f, 3.0f ),
