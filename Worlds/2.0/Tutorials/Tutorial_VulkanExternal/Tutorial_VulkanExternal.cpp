...
static void createVulkanInstance( Ogre::VulkanExternalInstance &outExternalInstance )
{
    ...
    for( size_t i = 0u; i < numExtensions; ++i )
    {
        ...
        LogManager::getSingleton().logMessage( "WORLD: Found instance extension: " + extensionName );
    ...
    for( size_t i = 0u; i < numInstanceLayers; ++i )
    {
        ...
        LogManager::getSingleton().logMessage( "WORLD: Found instance layer: " + layerName );
                                ...
                                Ogre::VulkanExternalDevice &outExternalDevice )
{
    ...
    if( queueFamilyIdx == std::numeric_limits<uint32_t>::max() )
    {
        LogManager::getSingleton().logMessage(
            "WORLD: No graphics queue found. Trying a compute one. Things may not work as expected." );
        for( uint32_t i = 0u; i < numQueueFamilies; ++i )
        {
        ...
        if( queueFamilyIdx == std::numeric_limits<uint32_t>::max() )
        {
            LogManager::getSingleton().logMessage( "WORLD: No graphics or compute queue found!" );
            abort();
        }
    }
    ...
    FastArray<const char *> deviceExtensions;
    {
        ...
        {
            const String extensionName = availableExtensions[i].extensionName;
            LogManager::getSingleton().logMessage( "WORLD: Found device extension: " + extensionName );
