...
namespace Final
{
    ...
    void Tutorial_SSAOGameState::createScene01()
    {
            ...
            for( int x = 0; x < numX; ++x )
            {
                for( int z = 0; z < numZ; ++z )
                {
                    ...
                    sceneNode->setPosition(
                        Ogre::Vector3( armsLengthSphere * Ogre::Real( x ) - startX,
                                       0.0f + Ogre::Real( numZ - z ) * 0.5f,
        ...
        for( size_t i = 0; i < 64u; ++i )
        {
            ...
            //            }
            Ogre::Vector3 sample = Ogre::Vector3( Ogre::Math::RangeRandom( -1.0f, 1.0f ),
                                                  Ogre::Math::RangeRandom( -1.0f, 1.0f ),
        ...
        Ogre::TextureGpu *noiseTexture = textureManager->createTexture(
            "noiseTexture", Ogre::GpuPageOutStrategy::SaveToSystemRam, 0, Ogre::TextureTypes::Type2D );
        ...
        psParams->setNamedConstant(
            "noiseScale",
            Ogre::Vector2(
                ( Ogre::Real( mGraphicsSystem->getRenderWindow()->getWidth() ) * 0.5f ) / 2.0f,
                ( Ogre::Real( mGraphicsSystem->getRenderWindow()->getHeight() ) * 0.5f ) / 2.0f ) );
    ...
    void Tutorial_SSAOGameState::keyReleased( const SDL_KeyboardEvent &arg )
    {
        ...
        else if( arg.keysym.sym == SDLK_F5 )
        {
            ...
            if( mKernelRadius < 0.05f )
                mKernelRadius = 0.05f;
        }
        else if( arg.keysym.sym == SDLK_F6 )
        {
            mKernelRadius = mKernelRadius + 0.1f;
            if( mKernelRadius > 10.0f )
                mKernelRadius = 10.0f;
        }
