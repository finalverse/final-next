...
namespace Final
{
    ...
    void AreaApproxLightsGameState::createAreaMask()
    {
        ...
        Ogre::Image2 image;
        image.loadDynamicImage( data, texWidth, texHeight, 1u, Ogre::TextureTypes::Type2D, texFormat,
                                true, 1u );
        for( size_t y = 0; y < texHeight; ++y )
        {
            for( size_t x = 0; x < texWidth; ++x )
            {
                if( ( y >= 10 && y <= texHeight - 11 ) && ( x >= 10 && x <= texWidth - 11 ) &&
                    ( ( y >= 10 && y <= 20 ) || ( x >= 10 && x <= 20 ) ||
        ...
        mAreaMaskTex =
            textureMgr->reservePoolId( c_areaLightsPoolId, image.getWidth(), image.getHeight(), 8u,
                                       image.getNumMipmaps(), image.getPixelFormat() );

        // Create the texture now, from the Image
        Ogre::TextureGpu *areaLightTex = textureMgr->createOrRetrieveTexture(
            "AreaLightMask0", Ogre::GpuPageOutStrategy::SaveToSystemRam,
            Ogre::TextureFlags::AutomaticBatching, Ogre::TextureTypes::Type2D, Ogre::BLANKSTRING, 0,
            c_areaLightsPoolId );
    ...
    Ogre::HlmsUnlitDatablock *AreaApproxLightsGameState::createPlaneForAreaLight( Ogre::Light *light )
    {
        ...
        const Ogre::String materialName =
            "LightPlane Material" + Ogre::StringConverter::toString( s_materialCounter++ );
        Ogre::HlmsMacroblock macroblock;
        macroblock.mCullMode = Ogre::CULL_NONE;
        Ogre::HlmsDatablock *datablockBase = hlmsUnlit->createDatablock(
            materialName, materialName, macroblock, Ogre::HlmsBlendblock(), Ogre::HlmsParamVec() );
        ...
        if( light->mTextureLightMaskIdx != std::numeric_limits<Ogre::uint16>::max() )
        {
            ...
            datablock->setTextureSwizzle(
                0, Ogre::HlmsUnlitDatablock::R_MASK, Ogre::HlmsUnlitDatablock::R_MASK,
                Ogre::HlmsUnlitDatablock::R_MASK, Ogre::HlmsUnlitDatablock::R_MASK );
        }
    ...
    void AreaApproxLightsGameState::createScene01()
    {
        ...
        Ogre::TextureGpu *areaTex = textureMgr->createOrRetrieveTexture(
            "AreaLightMask0", Ogre::GpuPageOutStrategy::AlwaysKeepSystemRamCopy,
            Ogre::TextureFlags::AutomaticBatching, Ogre::TextureTypes::Type2D, Ogre::BLANKSTRING, 0u,
            c_areaLightsPoolId );
        ...
        else if( arg.keysym.sym == SDLK_F3 )
        {
            for( int i = 0; i < 2; ++i )
            {
                mAreaLights[i]->setType( mAreaLights[i]->getType() == Ogre::Light::LT_AREA_APPROX
                                             ? Ogre::Light::LT_AREA_LTC
                                             : Ogre::Light::LT_AREA_APPROX );
