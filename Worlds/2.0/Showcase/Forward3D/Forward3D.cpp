...
namespace Final
{
    ...
    public:
        Forward3DGraphicsSystem( GameState *gameState ) : GraphicsSystem( gameState ) {}
    };
                                         ...
                                         GameState **outLogicGameState, LogicSystem **outLogicSystem )
    {
        Forward3DGameState *gfxGameState = new Forward3DGameState(
            "Forward3D & Clustered are techniques capable of rendering many lights. It is required in "
            "order\n"
            "to render non-shadow casting non-directional lights with the PBS implementation.\n"
            "Deferred shading has a lot of problems (transparency, antialiasing, multiple BDRF). "
            "Besides,\n"
            "it uses a lot of bandwidth. Forward+/Forward2.5 is great, but requires DX11 HW (needs UAV) "
            "and\n"
            "a Z-Prepass. This Z-prepass is often a turn off for many (even though in some cases it "
            "may\n"
            "improve performance, i.e. if you're heavily pixel shader or ROP [Raster OPeration] "
            "bound).\n"
            "It's not superior on all accounts, but it can work on DX10 hardware and doesn't require a\n"
            "Z-prepass. The result is a nice generic algorithm that can run on a lot of hardware and "
            "can\n"
            "handle a lot of lights. Whether it performs better or worse than Deferred or Forward+ "
            "depends\n"
            "on the scene.\n"
            "\n\n"
            "Like its alternatives Deferred & Forward+, it works best with many small lights, or few "
            "big\n"
            "lights.\n"
            "Forward3D & Clustered have many parameters, and this demo shows different presets that "
            "often\n"
            "trade quality for speed; but sometimes some presets work better on some scenarios than "
            "others.\n"
            ...
            "   6. Lights are 'lodded' based on distance to camera as a side effect of how the "
            "algorithm works\n"
    ...
    const char *MainEntryPoints::getWindowTitle() { return "Forward+ Lights"; }
}  // namespace Final
