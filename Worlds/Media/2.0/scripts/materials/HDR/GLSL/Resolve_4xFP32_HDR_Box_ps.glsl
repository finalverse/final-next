...
#if !MSAA_INITIALIZED
	#define MSAA_SUBWORLD_WEIGHT	0.25
	#define MSAA_NUM_SUBWORLDS		4
#endif
...
vec4 loadWithToneMapAndWeight( float invLum, texture2DMS tex, ivec2 iCoord, int subsample )
{
	...
	value.xyz = tonemapWithWeight( invLum, value.xyz, MSAA_SUBWORLD_WEIGHT );
	value.w *= MSAA_SUBWORLD_WEIGHT;
